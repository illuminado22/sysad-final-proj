...
backup_dirs() {
    ...
    # NEW: Adjust permissions for the demo
    # This allows the regular user to create/delete files in monitored_files
    # while keeping the backup folder strictly root-only (umask 077)
    chmod 700 "$main_backup" "$zip_archive"
    chmod 775 "$watch_dir"
    
    # If the user 'leroy' exists, ensure they own the monitored folder
    # This prevents "Permission Denied" when using touch/rm without sudo
...
# Backup recover uses 2 arguments
backup_recovered() {
    local filename="$1"
